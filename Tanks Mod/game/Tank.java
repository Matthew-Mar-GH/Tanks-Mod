...
public class Tank extends GameObject {
	...
	protected class TankCmd_ShootTwo extends TankCmd {
		static final String TYPE = "shoot2";
		public Vec2 dir;

		TankCmd_ShootTwo(Vec2 dir) {
			this.type = TankCmd_ShootTwo.TYPE;
			this.dir = dir.unit();
		}
	...
	protected class TankCmd_Nuke extends TankCmd {
		static final String TYPE = "nuke";
		public Vec2 dir;

		TankCmd_Nuke(Vec2 dir) {
			this.type = TankCmd_Nuke.TYPE;
			this.dir = dir.unit();
		}
	...
	protected void onCollect(PowerUp powerup) {
		...
		switch (powerup.getType()) {
			...
			case "P": // Points
				...
				break;
			case "T": // Turn Speed
				tankTurnSpeed += Game.POINTS_POWERUP_TURN;
				Util.log("(powerup: turn +" + Game.POINTS_POWERUP_TURN + "%)");
				break;
			case "N": // Turn Speed
				this.queuedCommands.add(new TankCmd_Nuke(this.dir));
				Util.log("(powerup: nuke +" + Game.POINTS_POWERUP_TURN + "%)");
				break;
			default:
	...
	private boolean queueCmd(String cmdStr, Vec2 param, boolean insertFront) {
		...
		} else if (cmdStr.toLowerCase().equals(TankCmd_ShootTwo.TYPE)) {
			TankCmd_ShootTwo cmdShootTwo = new TankCmd_ShootTwo(param);
			if (Vec2.lengthSqr(cmdShootTwo.dir) < 0.001) {
				Util.log("INVALID SHOT: Zero direction vector");
			} else {
				cmd = cmdShootTwo;
			}
	...
	protected void finishActiveCommand() {
		...
		} else if (nextCommand instanceof TankCmd_ShootTwo) {
			TankCmd_ShootTwo cmdShootTwo = (TankCmd_ShootTwo) nextCommand;
			if (Vec2.dot(this.dir.unit(), cmdShootTwo.dir.unit()) < 0.999) {
				nextCommand = new TankCmd_Turn(cmdShootTwo.dir.unit());
				needToTurnFirst = true;
			}
		} else if (nextCommand instanceof TankCmd_Nuke) {
			TankCmd_Nuke cmdNuke = (TankCmd_Nuke) nextCommand;
			if (Vec2.dot(this.dir.unit(), cmdNuke.dir.unit()) < 0.999) {
				nextCommand = new TankCmd_Turn(cmdNuke.dir.unit());
				needToTurnFirst = true;
			}
		}
		...
		int pointCost = (this.activeCommand.type.equals(TankCmd_Shoot.TYPE))
				? Game.POINTS_CMD_SHOT
				: (this.activeCommand.type.equals(TankCmd_ShootTwo.TYPE))
						? Game.POINTS_CMD_SHOT_TWO
						: Game.POINTS_CMD;
	...
	protected void updateCommand(double deltaTime) {
		...
		switch (this.activeCommand.type) {
			case TankCmd_Move.TYPE: {
				...
				// Check for collecting powerups on the way (might be moving fast, so swept
				// collision)...
				...
				for (int i = 0; i < gameObjects.size(); ++i) {
					...
					if (gameObject instanceof PowerUp) {
						...
						if (!powerUp.isDying() &&
								Util.intersectCircleCapsule(powerUp.pos, PowerUp.POWERUP_HALFDIMS.x, prevPos, motionVec,
										Tank.BODY_HALFSIZE.x)) {
							// Collect it...
			...
			case TankCmd_Shoot.TYPE: {
				...
				break;
			case TankCmd_ShootTwo.TYPE: {
				// Enforce a max shot period...
				if (timeSinceShot >= 1) {
					// Shoot...
					Simulation.get().createAmmoTwo(this.playerIdx, this.ammoSpawnLocation(), this.dir, this.ammoMaxRange);
				...
				break;
			case TankCmd_Nuke.TYPE: {
				// Enforce a max shot period...
				if (timeSinceShot >= 1) {
					// Shoot...
					Simulation.get().createNuke(this.playerIdx, this.ammoSpawnLocation(), this.dir, this.ammoMaxRange);
	...
	protected AffineTransform calcTransform(double height, double scale, boolean isShadow) {
		...
		Vec2 drawPos = isShadow ? Vec2.subtract(pos, Vec2.multiply(Draw.HEIGHT_OFFSET_PER_HEIGHT, drawHeight))
				: Vec2.add(pos, Vec2.multiply(Draw.HEIGHT_OFFSET_PER_HEIGHT, drawHeight));
	...
	protected void drawShadow(Graphics2D g) {
		...
		// Tread...
		Draw.drawRectShadow(g, transformTread, new Vec2(TREAD_HALFLENGTH, TREAD_HALFWIDTH), calcDrawScale(), colorShadow,
				TANK_ROUNDED_SIZE, new Vec2(0, TREAD_OFFSET));
		Draw.drawRectShadow(g, transformTread, new Vec2(TREAD_HALFLENGTH, TREAD_HALFWIDTH), calcDrawScale(), colorShadow,
				TANK_ROUNDED_SIZE, new Vec2(0, -TREAD_OFFSET));
	...
	protected void draw(Graphics2D g) {
		// Setup...
		double scale = calcDrawScale();
		Color colorBodyFill = Util.colorLerp(World.COLOR_BACKGROUND,
				playerIdx == 0 ? TANK_COLOR_BODY_FILL_1 : TANK_COLOR_BODY_FILL_2, timeSinceBorn * 2.0f);
		Color colorTreadFill = Util.colorLerp(World.COLOR_BACKGROUND, TANK_COLOR_TREAD_FILL, timeSinceBorn * 2.0f);
		Color colorTurretFill = Util.colorLerp(World.COLOR_BACKGROUND,
				playerIdx == 0 ? TANK_COLOR_TURRET_FILL_1 : TANK_COLOR_TURRET_FILL_2, timeSinceBorn * 2.0f);
		...
		// Tread...
		Draw.drawRect(g, transformTread, new Vec2(TREAD_HALFLENGTH, TREAD_HALFWIDTH), calcDrawScale(), colorTreadFill,
				colorStroke, 0, TANK_ROUNDED_SIZE, new Vec2(0, TREAD_OFFSET));
		Draw.drawRect(g, transformTread, new Vec2(TREAD_HALFLENGTH, TREAD_HALFWIDTH), calcDrawScale(), colorTreadFill,
				colorStroke, 0, TANK_ROUNDED_SIZE, new Vec2(0, -TREAD_OFFSET));

		// Body...
		Draw.drawRect(g, transformBody, BODY_HALFSIZE, calcDrawScale(), colorBodyFill, colorStroke, TANK_STROKE_WIDTH, 0,
				Vec2.zero());

		// Turret...
		Draw.drawRect(g, transformTurret, new Vec2(TURRET_HALFLENGTH, TURRET_HALFWIDTH), calcDrawScale(), colorTurretFill,
				colorStroke, TANK_TURRET_STROKE_WIDTH, TANK_ROUNDED_SIZE, new Vec2(TURRET_INSET, 0));
	}
}
